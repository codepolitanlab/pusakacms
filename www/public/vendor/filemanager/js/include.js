...
$(document).ready(function(){
		  ...
		  callback: function(key, options) {
		    switch (key) {
					...
					case "unzip":
					  ...
					  $.ajax({
							type: "POST",
							url: BASE_URL + "panel/media/ajax_calls/?action=extract",
							data: { path: m }
					  }).done(function( msg ) {
							if (msg!="")
						    bootbox.alert(msg);
    ...
    $('#filter-input').on('keyup',function(){
		    ...
		    if (js_script) {
					...
					lazyLoad();
					$.ajax({
						url: BASE_URL + "panel/media/ajax_calls/?action=filter&type="+val
					}).done(function( msg ) {
						if (msg!="") {
						  bootbox.alert(msg);
						}
    ...
    $('.sorter').on('click',function(){
			...
			if (js_script) {
				$.ajax({
					url: BASE_URL + "panel/media/ajax_calls/?action=sort&sort_by="+_this.attr('data-sort')+"&descending="+(sortDescending ? 1 : 0)
				});
	...
	bootbox.prompt($('#insert_folder_name').val(),$('#cancel').val(),$('#ok').val(), function(name) {
		...
		$.ajax({
			  type: "POST",
			  url: BASE_URL + "panel/media/execute/?action=create_folder",
    ...
    $('.view-controller button').on('click',function(){
	    ...
	    $.ajax({
				url: BASE_URL + "panel/media/ajax_calls/?action=view&type="+_this.attr('data-value')
			    }).done(function( msg ) {
				if (msg!="") {
				  bootbox.alert(msg);
				}
		...
		},
		{
			...
			"callback": function() {
                ...
                if (newFileName !== null)
                {
                	...
                	$.ajax({
					type: "POST",
					url: BASE_URL + "panel/media/execute/?action=create_file",
...
function edit_text_file($trigger) {
	...
	$.ajax({
	type: "POST",
	url: BASE_URL + "panel/media/ajax_calls/?action=get_file&sub_action=edit&preview_mode=text",
	data: {path: full_path}
			...
			[
				...
				},
				{
					...
					"callback": function() {
	                	...
	                	$.ajax({
						type: "POST",
						url: BASE_URL + "panel/media/execute/?action=save_text_file",
...
function change_lang() {
	$.ajax({
	type: "POST",
	url: BASE_URL + "panel/media/ajax_calls/?action=get_lang",
	data: {}
		...
		[
			...
			},
			{
				...
				"callback": function() {
                	...
                	$.ajax({
					type: "POST",
					url: BASE_URL + "panel/media/ajax_calls/?action=change_lang",
...
function chmod($trigger) {
	...
	$.ajax({
	type: "POST",
	url: BASE_URL + "panel/media/ajax_calls/?action=chmod",
			...
			},
			{
				...
				"callback": function() {
                    ...
                    if (newPerm != '' && typeof newPerm !== "undefined")
                    {
                    	...
                    	$.ajax({
						type: "POST",
						url: BASE_URL + "panel/media/execute/?action=chmod",
...
function clear_clipboard() {
	bootbox.confirm($('#lang_clear_clipboard_confirm').val(),$('#cancel').val(),$('#ok').val(), function(result) {
		if (result == true){
			$.ajax({
			type: "POST",
			url: BASE_URL + "panel/media/ajax_calls/?action=clear_clipboard",
			data: {}
			}).done(function( msg ) {
			if (msg!="")
			    bootbox.alert(msg);
...
function copy_cut_clicked($trigger, atype) {
    ...
    $.ajax({
	type: "POST",
	url: BASE_URL + "panel/media/ajax_calls/?action=copy_cut",
...
function paste_to_this_dir(dnd) {
	bootbox.confirm($('#lang_paste_confirm').val(),$('#cancel').val(),$('#ok').val(), function(result) {
		if (result == true){
			...
			$.ajax({
			type: "POST",
			url: BASE_URL + "panel/media/execute/?action=paste_clipboard",
			data: {path: folder_path, path_thumb: folder_path_thumb}
			}).done(function( msg ) {
				if (msg!=""){
					bootbox.alert(msg);
...
function close_window() {
	if ($('#popup').val()==1){
		window.close();
	}else if(typeof window.parent.closeBootstrapModal == 'function'){
		window.parent.closeBootstrapModal();
	} else {
...
function execute_action(action,file1,file2,name,container,function_name){
	...
	$.ajax({
	    type: "POST",
	    url: BASE_URL + "panel/media/execute/?action="+action,
