...
class Page extends Public_Controller {
	...
	function index()
	{
		...
		// if root domain called
		if (empty($segments))
		{
			if($this->pusaka->page_exist($this->config->item('homepage')))
				$this->page_detail(array($this->config->item('homepage')));
			else
			...
			if(count($segments) == 1 && $segments[0] == $this->config->item('homepage'))
				redirect('');
			...
			{
				$this->page_detail($segments);
			}
		}
	}

	function page_detail($segments)
	{
		$strseg = implode('/', $segments);
		if(! $page = $this->pusaka->get_page($strseg))
			show_404();
