...
if(!defined('BASEFM')) define('BASEFM', base_url().FMPATH);
	...
	|| !is_readable($_SESSION['RF']['language_file'])) 
{
	...
	$language_file = FMPATH.'lang/'.$default_language.'.php'; 
...
<html xmlns="https://www.w3.org/1999/xhtml">
  ...
  <link href="<?php  echo vendor_url('filemanager', 'css/style.css'); ?>" rel="stylesheet" type="text/css" />
	...
	if ( ! file_exists(FMPATH.$sprite_lang_file) || ! file_exists(FMPATH.$sprite_lang_file2)){
		//fallback
	...
	<style>
		...
		.dropzone-previews .dz-preview .dz-progress .dz-upload {
			background-image: url(<?php echo BASEFM.$sprite_lang_file; ?>);
		}
	...
	<script type="text/javascript" src="<?php echo BASEFM; ?>js/include.js"></script>
</head>
<body>
...
<div class="navbar navbar-fixed-top">
    <div class="navbar-inner">
		...
		<div class="filters">
			...
			<div class="span2 half view-controller" style="text-align:center">
			...
			<div class="span6 half types" style="display:block"> 
				<span><?php echo lang_Filters; ?>:</span>
			    <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
			    <label id="ff-item-type-all" title="<?php echo lang_All; ?>" <?php if($_GET['type']==1 || $_GET['type']==3){ ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-right:0px;" class="tip btn btn-inverse ff-label-type-all"><i class="icon-align-justify icon-white"></i></label>
			    ...
			    <input style="margin-right:-8px;width:150px" accesskey="f" type="text" class="filter-input <?php echo (($_GET['type']!=1 && $_GET['type']!=3) ? '' : 'filter-input-notype'); ?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(lang_Text_filter); ?>..." value="<?php echo $filter; ?>"/><?php if($n_files>$file_number_limit_js){ ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>			    
			</div>
		    </div>
		</div>
	    </div>
		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
			...
			$file=$file_array['file'];
			    ...
			    if($src_thumb==""){
			    	$no_thumb=true;
			    	if(file_exists(FMPATH.'img/'.$icon_theme.'/'.$extension_lower.".jpg")){
			    		$src_thumb =FMPATH.'img/'.$icon_theme.'/'.$extension_lower.".jpg";
			    	}else{
			    		$src_thumb = FMPATH."img/".$icon_theme."/default.jpg";
			    	}
			    	$is_icon_thumb=true;
			    }
			    if($mini_src==""){
            ...
            ?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<div class="img-precontainer">
				    ...
				    <div class="img-container">
					    <span></span>
					    <img class="<?php echo $show_original ? "original" : "" ?><?php echo $is_icon_thumb ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : ""?>" <?php echo $lazy_loading_enabled ? "data-original" : "src"?>="<?php echo base_url($src_thumb); ?>">
				    </div>
				</div>
				<div class="img-precontainer-mini <?php if($is_img) echo 'original-thumb' ?>">
					...
					<span></span>
					<?php if($mini_src!=""){ ?>
					<img class="<?php echo $show_original_mini ? "original" : "" ?><?php echo $is_icon_thumb_mini ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : ""?>" <?php echo $lazy_loading_enabled ? "data-original" : "src"?>="<?php echo base_url($mini_src); ?>">
					<?php } ?>
				    </div>
				</div>
				...
				<figcaption>
				    <form action="<?php echo site_url('panel/media/force_download'); ?>" method="post" class="download-form" id="form<?php echo $nu; ?>">
...
<script>
    ...
    <div id="loading_container" style="display:none;">
	    ...
	    <img id="loading_animation" src="<?php echo BASEFM; ?>img/storing_animation.gif" alt="loading" style="z-index:10001; margin-left:-32px; margin-top:-32px; position:fixed; left:50%; top:50%"/>
    </div>
    <!-- loading div end -->
    ...
    <?php if ($lazy_loading_enabled) { ?>
        <script src="<?php echo BASEFM; ?>js/jquery.lazyload.min.js" type="text/javascript"></script>
        <script src="<?php echo BASEFM; ?>js/jquery.scrollstop.min.js" type="text/javascript"></script>
